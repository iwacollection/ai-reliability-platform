...
VERSION = "investigation-execution-resilience-v2"

AFTER_NAME = (
    "investigation_execution_resilience_v2_after.txt"
)

ERROR_NAME = (
    "investigation_execution_resilience_v2_error.txt"
)
...
def main() -> int:
    ...
    report = [
        "Investigation Execution Resilience v2",
        f"GeneratedAt: {datetime.now().astimezone().isoformat()}",
        "",
        "Installer v2 correction:",
        "- fixes the focused test to split the real newline-delimited State JSON marker instead of the literal backslash-n sequence",
        "- Execution Resilience runtime behavior is unchanged from v1",
        "",
    ...
    try:
        ...
        report.extend(
            [
                "PASSED",
                "",
                "Execution Resilience v2 is installed.",
                "",
        ...
        print("=" * 72)
        print(
            "INVESTIGATION EXECUTION RESILIENCE V2 PASSED"
        )
        print("=" * 72)
        print("")
        print(
        ...
        write_text(
            ...
            "\n".join(
                [
                    "Investigation Execution Resilience v2 FAILED",
                    f"GeneratedAt: {datetime.now().astimezone().isoformat()}",
                    "",
                    "Exception:",
                    f"{type(exc).__name__}: {exc}",
