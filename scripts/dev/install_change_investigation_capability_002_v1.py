...
VERSION = "change-investigation-capability-002-v1"

AFTER_NAME = (
    "change_investigation_capability_002_v1_after.txt"
)

ERROR_NAME = (
    "change_investigation_capability_002_v1_error.txt"
)

EXPECTED_HASHES = {'services/agent_runtime/app/investigation/models.py': '673a8dbe607782dd8316fe1c9b7ae85d716ba6077da5ae67a31ce8d2c665614d', 'services/agent_runtime/app/investigation/reasoner.py': '4931b9f9228c86a0b9c2552ea5378a884da04d79334e1ebed746c7f408348df6', 'services/agent_runtime/app/investigation/coordinator.py': '3bbaa571e749ad07eb2be560ef34935e6d903dcaabd4b7878564bf8d933d6079', 'services/agent_runtime/app/investigation/probes.py': '6fda3e2605d704790ba015414dd87a02847c1cd5d1046a0cd8d6a3af7bcf860b', 'services/agent_runtime/app/investigation/epistemic_guard.py': '70dbb396b1920e18979b550cee02939a42c92626cf0c82cffccfdc9099ecdc53', 'services/agent_runtime/app/tools/factory.py': 'e0ff735e25b493acbd62d4c59150f3bcf45c2baff3ebb7ddb005b55658d4d27d'}
...
def verify_hash(
    *,
    ...
    if actual != expected:
        raise RuntimeError(
            (
                f"{relative} changed after the reviewed current baseline. "
                f"expected_sha256={expected} actual_sha256={actual}. "
                "Refusing stale Change Investigation installation."
            )
        )
...
def patch_tools_factory(
    ...
    if (
        "KubernetesChangeTool"
        in text
        or "kubernetes_change"
        in text
    ):
        raise RuntimeError(
            "tools/factory.py already contains Change Tool wiring"
        )
    ...
    kubernetes_imports = [
        ...
        if (
            ...
            and node.module
            == (
                "services.agent_runtime.app.tools."
                "kubernetes.tool"
            )
            and any(
                alias.name
    ...
    if len(
        ...
        raise RuntimeError(
            "Could not locate exact KubernetesTool import"
        )
    ...
    if len(
        ...
        raise RuntimeError(
            "Could not locate create_tool_manager"
        )

    function = factory_functions[
        0
    ]
    ...
    if len(
        ...
        raise RuntimeError(
            "Could not locate exact KubernetesTool registration"
        )
    ...
    import_node = kubernetes_imports[
        0
    ]

    register_node = registrations[
        0
    ]

    import_insert_at = offsets[
        import_node.end_lineno
    ]

    register_start = offsets[
        register_node.lineno - 1
    ]

    register_end = offsets[
        register_node.end_lineno
    ]
...
def main() -> int:
    ...
    report = [
        "Change Investigation Capability #002 v1",
        f"GeneratedAt: {datetime.now().astimezone().isoformat()}",
        "",
        "Capability:",
        "- adds symbolic kubernetes_workload_change Investigation probe",
        "- adds capability-gated available_probes",
        "- adds a real read-only kubernetes_change Tool",
        "- resolves only a verified Pod -> ReplicaSet -> Deployment controller chain",
        "- reads bounded ReplicaSet history through a Deployment-owned selector",
        "- emits bounded revision/image/rollout/generation/replica facts",
        "",
        "Safety:",
        "- only fixed GET requests are used",
        "- LLM cannot provide Kubernetes verb/resource/path/selector",
        "- no dry-run/mock evidence is accepted as production change evidence",
        "- unsupported owner chains fail closed",
        "- RBAC denial fails closed",
        "- change evidence alone is rejected as causal proof by Epistemic Guard",
        "- no Action/Approval/Verification authority is added",
        "",
        "Compatibility:",
        "- default InvestigationState retains only the existing five baseline probes",
        "- Change probe is exposed only when kubernetes_change exists and reports available",
        "- custom benchmark executors without this capability stay on the original vocabulary",
        "",
    ...
    try:
        ...
        if syntax.returncode != 0:
            raise RuntimeError(
                "Change Investigation syntax verification failed"
            )

        focused = run_command(
            root=root,
            name="Change Investigation focused suite",
            command=[
                "uv",
                "run",
                "pytest",
        ...
        if focused.returncode != 0:
            raise RuntimeError(
                "Change Investigation focused tests failed"
            )

        compatibility = run_command(
            root=root,
            name="Investigation capability compatibility suite",
            command=[
                "uv",
                "run",
                "pytest",
        ...
        if compatibility.returncode != 0:
            raise RuntimeError(
                "Change Investigation compatibility tests failed"
            )

        preflight = run_command(
            root=root,
            name="Change capability preflight",
            command=[
                "uv",
                "run",
                "python",
                ...
                (
                    ...
                    "import InvestigationProbe, InvestigationState, InvestigationScope; "
                    "from services.agent_runtime.app.tools.factory import create_tool_manager; "
                    "s=InvestigationState(scope=InvestigationScope("
                    "alert_name='A',resource='p')); "
                    "m=create_tool_manager(); "
                    "names=[x.name for x in m.registry.list_tools()]; "
                    "print('change_enum='+InvestigationProbe.KUBERNETES_WORKLOAD_CHANGE.value); "
                    "print('default_change_enabled='+str("
                    "InvestigationProbe.KUBERNETES_WORKLOAD_CHANGE in s.available_probes)); "
                    "print('tools='+str(names)); "
                    "assert InvestigationProbe.KUBERNETES_WORKLOAD_CHANGE not in s.available_probes; "
                    "assert 'kubernetes_change' in names"
                ),
            ],
        )
        ...
        if preflight.returncode != 0:
            raise RuntimeError(
                "Change capability preflight failed"
            )
        ...
        authority = run_command(
            ...
            command=[
                ...
                (
                    ...
                    "'.patch(','.post(','.put(','.delete('] if x in s]; "
        ...
        if authority.returncode != 0:
            raise RuntimeError(
                "Change Investigation read-only authority boundary failed"
            )
        ...
        report.extend(
            [
                "PASSED",
                "",
                "Change Investigation Capability #002 v1 is installed.",
                "",
                "Available now:",
                "- capability-gated kubernetes_workload_change symbolic probe",
                "- verified Pod -> ReplicaSet -> Deployment read-only owner-chain traversal",
                "- bounded current/previous rollout revision context",
                "- bounded image before/after comparison",
                "- incident-relative rollout timing",
                "- generation/observedGeneration and replica rollout state",
                "- deterministic Guard preventing change-only causal RCA",
                "",
                "Not yet claimed by v1:",
                "- ConfigMap/Secret diff",
                "- StatefulSet/DaemonSet rollout history",
                "- Git/CI/CD author or commit provenance",
                "- arbitrary Kubernetes Events search",
                "",
                "Next:",
                "add dedicated Change Intelligence benchmark scenarios before expanding the change evidence surface.",
            ]
        )
        ...
        print("=" * 72)
        print(
            "CHANGE INVESTIGATION CAPABILITY #002 V1 PASSED"
        )
        print("=" * 72)
        print("")
        print(
        ...
        write_text(
            ...
            "\n".join(
                [
                    "Change Investigation Capability #002 v1 FAILED",
                    f"GeneratedAt: {datetime.now().astimezone().isoformat()}",
                    "",
