...
VERSION = "change-intelligence-benchmark-v2"

AFTER_NAME = (
    "change_intelligence_benchmark_v2_after.txt"
)

ERROR_NAME = (
    "change_intelligence_benchmark_v2_error.txt"
)

EXPECTED_HASHES = {'services/agent_runtime/app/evaluation/intelligence_benchmark/engine.py': '531fc9f6de21100e7ed45dfc3a73d6eb1dcaef2c97069f2d4555488faa169bdf', 'services/agent_runtime/app/evaluation/intelligence_benchmark/scenarios.py': '5d5c7d6a6bf8dd6ff09d6721708dd9de271f65f591a05283422caecc3ebb97d0', 'services/agent_runtime/app/investigation/models.py': '2cbd6ecfc0929b4297b3ffd714e8f7415f9ed4ff9857fbb934933dc07fb8637d', 'services/agent_runtime/app/investigation/epistemic_guard.py': '7fbd2ccb99c9fb3fda4d9172facc6ce9b3d7b807e8f01ab3dadefcf8c33792cb'}
    ...
    if actual != expected:
        raise RuntimeError(
            (
                ...
                "Refusing stale Change Benchmark installation."
            )
        )
...
def main() -> int:
    ...
    report = [
        "Change Intelligence Benchmark v2",
        f"GeneratedAt: {datetime.now().astimezone().isoformat()}",
        "",
        "v2 correction:",
        "- fixes a real benchmark-domain boundary bug where scenario_key + probe_name could exceed EvidenceItem.evidence_id max_length=64",
        "- preserves every historical evidence ID that already fits within 64 characters",
        "- compacts only overlong IDs with a deterministic readable prefix + SHA-256 suffix",
        "- Change tests consume the same benchmark_evidence_id helper as the executor",
        "",
        "Purpose:",
        "- evaluate Change Investigation Capability #002 with hidden-label synthetic evidence",
        "- keep the historical Full 7 and Smoke 3 benchmark sets unchanged",
        "- expose the Change probe only to scenarios that explicitly contain Change evidence",
        "- test correlation-vs-causation discipline before expanding the evidence surface",
        "",
        "Five Change exams:",
        "- recent image rollout + causal previous logs -> sufficient_evidence",
        "- recent rollout + genuine OOM pressure -> memory RCA, not rollout RCA",
        "- stale three-day-old rollout + unexplained CrashLoop -> abstain",
        "- unavailable Change backend -> safe abstain, never invent change facts",
        "- recent rollout + failure symptom but no mechanism evidence -> abstain / Guard rescue",
        "",
        "Safety:",
        "- hidden evaluator labels never enter EvidenceItem facts or Agent prompt",
        "- benchmark-only files are changed",
        "- no Runtime/Tool/Reasoner/Guard authority change",
    ...
    try:
        ...
        if syntax.returncode != 0:
            raise RuntimeError(
                "Change Intelligence Benchmark syntax failed"
            )

        focused = run_command(
            root=root,
            name="Change Intelligence Benchmark focused suite",
            command=[
                "uv",
                "run",
                "pytest",
        ...
        if focused.returncode != 0:
            raise RuntimeError(
                "Change Intelligence Benchmark focused tests failed"
            )

        compatibility = run_command(
            root=root,
            name="Change / Investigation compatibility suite",
            command=[
                "uv",
                "run",
                "pytest",
        ...
        if compatibility.returncode != 0:
            raise RuntimeError(
                "Change Intelligence compatibility tests failed"
            )

        preflight = run_command(
            root=root,
            name="Change scenario isolation preflight",
            command=[
                "uv",
                "run",
                "python",
                ...
                (
                    ...
                    "SCENARIOS,CHANGE_SCENARIOS,CHANGE_SCENARIO_KEYS,"
                    "scenarios_for_mode,scenario_by_key; "
                    "print('baseline_full='+str(len(scenarios_for_mode('full')))); "
                    "print('baseline_smoke='+str(len(scenarios_for_mode('smoke')))); "
                    "print('change_count='+str(len(CHANGE_SCENARIOS))); "
                    "print('change_keys='+str(CHANGE_SCENARIO_KEYS)); "
                    "assert len(SCENARIOS)==7; "
                    "assert len(scenarios_for_mode('full'))==7; "
                    "assert len(scenarios_for_mode('smoke'))==3; "
                    "assert len(CHANGE_SCENARIOS)==5; "
                    "assert all(scenario_by_key(k).key==k for k in CHANGE_SCENARIO_KEYS)"
                ),
            ],
        )
        ...
        if preflight.returncode != 0:
            raise RuntimeError(
                "Change scenario isolation preflight failed"
            )
        ...
        authority = run_command(
            ...
            command=[
                ...
                (
                    ...
                    "intelligence_benchmark/engine.py'),"
                    "Path(r'services/agent_runtime/app/evaluation/"
                    "intelligence_benchmark/scenarios.py')"
                    "]; "
                    "s='\\n'.join(x.read_text(encoding='utf-8') for x in files); "
                    "bad=[x for x in ['ActionRuntime','ApprovalService',"
                    "'VerificationRuntime','.post(','.patch(','.put(','.delete('] "
                    "if x in s]; "
        ...
        if authority.returncode != 0:
            raise RuntimeError(
                "Change Benchmark authority boundary failed"
            )
        ...
        report.extend(
            [
                "PASSED",
                "",
                "Change Intelligence Benchmark v2 is installed.",
                "",
                "Historical benchmark sets remain:",
                "- Smoke = 3 scenarios",
                "- Full = 7 scenarios",
                "",
                "Dedicated Change suite:",
                "- 5 scenarios addressable through scenario_by_key",
                "- existing batch v2 runner can execute all 5 x N and emit one bundle",
                "",
                ...
                (
                    ...
                    "--provider bailian "
                    "--scenario change_image_rollout_log_rca "
                    "--scenario recent_change_but_memory_pressure "
                    "--scenario stale_rollout_crashloop_unknown "
                    "--scenario change_probe_backend_failure "
                    "--scenario recent_change_without_causal_mechanism "
                    "--repeat 3 "
                    "--output investigation_change_intelligence_bundle.json"
                ),
            ]
        )
        ...
        print("=" * 72)
        print(
            "CHANGE INTELLIGENCE BENCHMARK V2 PASSED"
        )
        print("=" * 72)
        print("")
        print(
        ...
        write_text(
            ...
            "\n".join(
                [
                    "Change Intelligence Benchmark v2 FAILED",
                    f"GeneratedAt: {datetime.now().astimezone().isoformat()}",
                    "",
