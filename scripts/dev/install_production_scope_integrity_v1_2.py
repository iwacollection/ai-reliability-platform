...
VERSION = "production-scope-integrity-v1.2"

AFTER_NAME = (
    "production_scope_integrity_v1_2_after.txt"
)

ERROR_NAME = (
    "production_scope_integrity_v1_2_error.txt"
)

EXPECTED_PRODUCTION_HASHES = {'services/agent_runtime/app/investigation/probes.py': 'c85c8850bdb88c8f1d30302a5c3288659e5051849966d991db18d6bbc1391b39', 'services/agent_runtime/app/tools/kubernetes/tool.py': 'f9ceb57e457ad68a25c7c4da03476cad552cd45e484310cf02dd9993197f8424'}
...
PROBE_TEST_OLD = '                "target": \'payment"api\',\n                "namespace": "team\\\\blue",\n            },\n'
PROBE_TEST_NEW = '                "target": \'payment"api\',\n                "namespace": "team\\\\blue",\n                "cluster": "prod\\nwest",\n            },\n'

LOGS_TEST_OLD = '                "target": "payment-api",\n                "namespace": "payment",\n            },\n'
LOGS_TEST_NEW = '                "target": "payment-api",\n                "namespace": "payment",\n                "cluster": "benchmark-lab",\n            },\n'
...
def verify_production_hash(
    *,
    ...
    if not path.exists():
        raise RuntimeError(
            f"Required production file is missing: {relative}"
        )
    ...
    expected = EXPECTED_PRODUCTION_HASHES[
        relative
    ]
    ...
    if actual != expected:
        raise RuntimeError(
            (
                f"{relative} changed after the reviewed production baseline. "
                f"expected_sha256={expected} actual_sha256={actual}. "
                "Refusing stale Production Scope Integrity v1.2 installation."
            )
        )


def patch_exactly_once(
    *,
    path: Path,
    old: str,
    new: str,
    ...
    if count != 1:
        raise RuntimeError(
            (
                f"{label} patch anchor count must be exactly 1; "
                f"actual={count}; file={path}"
            )
        )
    ...
    if new in current:
        raise RuntimeError(
            (
                f"{label} appears already patched while old anchor is still present; "
                "refusing ambiguous test mutation"
            )
        )
...
def main() -> int:
    ...
    report = [
        "Production Scope Integrity Contract v1.2",
        f"GeneratedAt: {datetime.now().astimezone().isoformat()}",
        "",
        "v1.1 preflight diagnosis:",
        "- no source file was modified because v1.1 stopped during SHA256 preflight",
        "- production probes.py and kubernetes/tool.py hashes still match the reviewed baseline",
        "- only test_investigation_probes.py had evolved relative to the stale embedded test snapshot",
        "",
        "v1.2 stale-source policy:",
        "- production files remain protected by exact SHA256 preflight",
        "- legacy tests are NOT replaced wholesale",
        "- each legacy test receives one exact assertion-block patch only",
        "- if an expected old assertion block is missing or duplicated, installation fails closed",
        "",
        "Production contract:",
        "- Alertmanager cluster/namespace/pod survives Parser -> StandardEvent -> InvestigationScope",
        "- Pod State forwards resource + namespace + cluster",
        "- Previous Logs forwards resource + namespace + cluster",
        "- Prometheus selectors preserve resource + namespace + cluster",
        "- KubernetesTool rejects requested/configured cluster mismatch before HTTP",
        "- two cluster scopes cannot bleed into each other",
        "",
        "Safety:",
        "- no multi-cluster router yet",
        "- no write Kubernetes verb",
    ...
    try:
        section(
            report,
            "PRODUCTION HASH PREFLIGHT",
        )

        for relative in EXPECTED_PRODUCTION_HASHES:
        ...
        if not probe_test_file.exists():
            raise RuntimeError(
                "Existing probe contract test is missing"
            )

        if not logs_test_file.exists():
            raise RuntimeError(
                "Existing logs contract test is missing"
            )

        probe_test_current = read_text(
            probe_test_file
        )

        logs_test_current = read_text(
            logs_test_file
        )
        ...
        report.extend(
            [
                "",
                (
                    "test_investigation_probes.py_current_sha256="
                    + sha256_text(
                        probe_test_current
                    )
                ),
                (
                    "test_investigation_logs.py_current_sha256="
                    + sha256_text(
                        logs_test_current
                    )
                ),
                (
                    "probe_test_old_anchor_count="
                    + str(
                        probe_test_current.count(
                            PROBE_TEST_OLD
                        )
                    )
                ),
                (
                    "logs_test_old_anchor_count="
                    + str(
                        logs_test_current.count(
                            LOGS_TEST_OLD
                        )
                    )
                ),
        ...
        if (
            ...
            raise RuntimeError(
                "Probe legacy test does not contain exactly one reviewed pre-cluster assertion block"
            )
        ...
        if (
            ...
            raise RuntimeError(
                "Logs legacy test does not contain exactly one reviewed pre-cluster assertion block"
            )
        ...
        probe_before, probe_after = (
            patch_exactly_once(
                path=probe_test_file,
                old=PROBE_TEST_OLD,
                new=PROBE_TEST_NEW,
                label="Pod State cluster assertion",
            )
        )
        ...
        logs_before, logs_after = (
            patch_exactly_once(
                ...
                old=LOGS_TEST_OLD,
                new=LOGS_TEST_NEW,
                label="Previous Logs cluster assertion",
            )
        )

        section(
            report,
            "LEGACY TEST PATCH HASHES",
        )

        report.extend(
            [
                (
                    "test_investigation_probes.py_before="
                    + probe_before
                ),
                (
                    "test_investigation_probes.py_after="
                    + probe_after
                ),
                (
                    "test_investigation_logs.py_before="
                    + logs_before
                ),
                (
                    "test_investigation_logs.py_after="
                    + logs_after
                ),
            ]
        )
        ...
        if syntax.returncode != 0:
            raise RuntimeError(
                "Production Scope Integrity v1.2 syntax failed"
            )
        ...
        if focused.returncode != 0:
            raise RuntimeError(
                "Production Scope Integrity v1.2 focused tests failed"
            )
        ...
        if compatibility.returncode != 0:
            raise RuntimeError(
                "Production Scope Integrity v1.2 compatibility tests failed"
            )

        propagation = run_command(
            root=root,
            ...
            command=[
                ...
                (
                    ...
                    ".read_text(encoding='utf-8'); "
                    "tp=Path(r'services/agent_runtime/tests/test_investigation_probes.py')"
                    ".read_text(encoding='utf-8'); "
                    "tl=Path(r'services/agent_runtime/tests/test_investigation_logs.py')"
                    ".read_text(encoding='utf-8'); "
                    ...
                    "print('pod_test_cluster='+str('\"cluster\": \"prod\\\\nwest\"' in tp)); "
                    "print('logs_test_cluster='+str('\"cluster\": \"benchmark-lab\"' in tl)); "
                    ...
                    "assert '\"cluster\": \"prod\\\\nwest\"' in tp; "
                    "assert '\"cluster\": \"benchmark-lab\"' in tl"
                ),
            ],
        )
        ...
        if propagation.returncode != 0:
            raise RuntimeError(
                "Production Scope Integrity v1.2 propagation preflight failed"
            )
        ...
        if authority.returncode != 0:
            raise RuntimeError(
                "Production Scope Integrity v1.2 authority boundary failed"
            )
        ...
        report.extend(
            [
                "PASSED",
                "",
                "Production Scope Integrity Contract v1.2 is installed.",
                "",
                "Current guarantee:",
                "- arbitrary production event scope is preserved",
                "- Pod State and Previous Logs retain cluster identity",
                "- Prometheus retains cluster identity",
                "- cluster-bound KubernetesTool fails closed on cross-cluster mismatch before HTTP",
                "",
                "Still intentionally not implemented:",
                "- multi-cluster client selection/router",
                "",
                "Next:",
        ...
        print("=" * 72)
        print(
            "PRODUCTION SCOPE INTEGRITY CONTRACT V1.2 PASSED"
        )
        print("=" * 72)
        print("")
        print(
        ...
        write_text(
            ...
            "\n".join(
                [
                    "Production Scope Integrity Contract v1.2 FAILED",
                    f"GeneratedAt: {datetime.now().astimezone().isoformat()}",
                    "",
