...
AFTER_NAME = "real_llm_connectivity_preflight_v2_after.txt"
ERROR_NAME = "real_llm_connectivity_preflight_v2_error.txt"
...
def install_repo_import_paths(root: Path) -> None:
    """
    Dev scripts live under scripts/dev, so Python normally places that
    directory at sys.path[0] instead of the repository root.

    Add only the two project import roots needed by this repository:

    - repository root      -> services.*
    - packages/common/src  -> common.*

    No global PYTHONPATH mutation is performed.
    """
...
async def run_live_investigation_preflight(
    *,
    root: Path,
) -> dict:
    """
    Send exactly one Investigation reasoning request.

    Imports happen only after repository import roots have been installed.
    """
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Run only local checks/tests and do not send "
            "the real LLM request."
        ),
    )
    ...
    # Install repository import paths before any in-process product import.
    install_repo_import_paths(
        root
    )
    ...
    report = [
        "Real LLM Connectivity + Investigation JSON Contract Preflight v2",
        f"GeneratedAt: {datetime.now().astimezone().isoformat()}",
        "",
        "Fix from v1:",
        "- repository root is explicitly added to sys.path",
        "- packages/common/src is explicitly added to sys.path",
        "- this fixes scripts/dev execution without changing product PYTHONPATH",
        "",
        "Safety:",
        "- configs/app.yaml must remain provider=mock",
    ...
    try:
        ...
        import_check = run_command(
            root=root,
            name="Repository import preflight",
            command=[
                "uv",
                "run",
                "python",
                ...
                (
                    ...
                    "print('services_import=ok'); "
                    "print('common_import=ok'); "
                    "print('provider='+get_settings().llm.provider)"
                ),
            ],
        )
        ...
        if (
            ...
            raise RuntimeError(
                "Repository import preflight failed"
            )
        ...
        print("=" * 72)
        print(
            "REAL LLM CONNECTIVITY PREFLIGHT V2 PASSED"
        )
        print("=" * 72)
        print("")
        print(
        ...
        print("=" * 72)
        print(
            "REAL LLM CONNECTIVITY PREFLIGHT V2 FAILED"
        )
        print("=" * 72)
        print("")
        print("Upload:")
