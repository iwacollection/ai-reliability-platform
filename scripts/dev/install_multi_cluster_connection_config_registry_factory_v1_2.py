...
VERSION = "multi-cluster-connection-config-registry-factory-v1.2"

AFTER_NAME = (
    "multi_cluster_connection_config_registry_factory_v1_2_after.txt"
)

ERROR_NAME = (
    "multi_cluster_connection_config_registry_factory_v1_2_error.txt"
)
...
def main() -> int:
    ...
    report = [
        "Multi-Cluster Connection Config / Registry Factory v1.2",
    ...
    try:
        ...
        missing_required_settings_tests = [
            str(
                path.relative_to(
                    root
                )
            )
            for path in required_settings_tests
            if not path.exists()
        ]

        if missing_required_settings_tests:
            raise RuntimeError(
                "Required settings/security compatibility tests are missing: "
                + ", ".join(
                    missing_required_settings_tests
                )
            )
        ...
        report.extend(
            [
                "PASSED",
                "",
                "Multi-Cluster Connection Config / Registry Factory v1.2 is installed.",
                "",
        ...
        print(
            "MULTI-CLUSTER CONNECTION CONFIG / REGISTRY FACTORY V1.2 PASSED"
        )
        print(
            "=" * 72
        )
        ...
        write_text(
            ...
            "\n".join(
                [
                    "Multi-Cluster Connection Config / Registry Factory v1.2 FAILED",
                    (
                        "GeneratedAt: "
