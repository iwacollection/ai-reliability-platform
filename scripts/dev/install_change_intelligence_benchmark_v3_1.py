...
VERSION = "change-intelligence-benchmark-v3.1"

AFTER_NAME = (
    "change_intelligence_benchmark_v3_1_after.txt"
)

ERROR_NAME = (
    "change_intelligence_benchmark_v3_1_error.txt"
)
...
def main() -> int:
    ...
    report = [
        "Change Intelligence Benchmark v3.1",
    ...
    try:
        ...
        if syntax.returncode != 0:
            raise RuntimeError(
                "Change Benchmark v3.1 syntax failed"
            )
        ...
        if compatibility.returncode != 0:
            raise RuntimeError(
                "Change Benchmark v3.1 compatibility tests failed"
            )
        ...
        preflight = run_command(
            ...
            command=[
                ...
                (
                    ...
                    ").collect(None,None,InvestigationProbe.KUBERNETES_WORKLOAD_CHANGE)).facts) "
                    "for s in CHANGE_ROLLOUT_EVENT_SCENARIOS]; "
                    "print('facts_counts='+str(counts)); "
        ...
        if preflight.returncode != 0:
            raise RuntimeError(
                "Change Benchmark v3.1 preflight failed"
            )
        ...
        if authority.returncode != 0:
            raise RuntimeError(
                "Change Benchmark v3.1 authority boundary failed"
            )
        ...
        report.extend(
            [
                "PASSED",
                "",
                "Change Intelligence Benchmark v3.1 is installed.",
                "",
        ...
        print("=" * 72)
        print(
            "CHANGE INTELLIGENCE BENCHMARK V3.1 PASSED"
        )
        print("=" * 72)
        print("")
        print(
        ...
        write_text(
            ...
            "\n".join(
                [
                    "Change Intelligence Benchmark v3.1 FAILED",
                    f"GeneratedAt: {datetime.now().astimezone().isoformat()}",
                    "",
