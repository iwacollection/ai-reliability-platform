...
VERSION = "production-scope-integrity-v1"

AFTER_NAME = (
    "production_scope_integrity_v1_after.txt"
)

ERROR_NAME = (
    "production_scope_integrity_v1_error.txt"
)

EXPECTED_HASHES = {'services/agent_runtime/app/investigation/probes.py': 'c85c8850bdb88c8f1d30302a5c3288659e5051849966d991db18d6bbc1391b39', 'services/agent_runtime/app/tools/kubernetes/tool.py': 'f9ceb57e457ad68a25c7c4da03476cad552cd45e484310cf02dd9993197f8424'}
    ...
    if actual != expected:
        raise RuntimeError(
            (
                ...
                "Refusing stale Production Scope Integrity installation."
            )
        )
...
def main() -> int:
    ...
    report = [
        "Production Scope Integrity Contract v1",
        f"GeneratedAt: {datetime.now().astimezone().isoformat()}",
        "",
        "Why:",
        "- Demo payment/payment-api values are fixtures, not production routing defaults",
        "- production is expected to contain multiple clusters/namespaces/workloads",
        "- cluster scope must never be silently dropped before a Kubernetes read",
        "",
        "Contract:",
        "- Alertmanager payload cluster/namespace/pod must survive parsing into StandardEvent",
        "- StandardEvent resource scope must survive Coordinator scope construction",
        "- Kubernetes Pod State and Previous Logs must receive resource + namespace + cluster",
        "- Prometheus queries must retain resource + namespace + cluster",
        "- a KubernetesTool bound to one cluster must reject a different requested cluster before HTTP",
        "- sequential events from different clusters must not bleed scope",
        "",
        "Architecture boundary:",
        "- this v1 does NOT implement a multi-cluster router",
        "- it makes the existing single-cluster KubernetesTool cluster-aware and fail-closed",
        "- future work may map scope.cluster to a cluster-specific Tool/client",
        "",
        "Safety:",
        "- no write Kubernetes verb added",
        "- no Action / Approval / Verification change",
        "- no real Kubernetes / Prometheus / LLM request",
        "- tests use in-memory parser inputs and httpx.MockTransport only",
    ]
    ...
    try:
        ...
        if syntax.returncode != 0:
            raise RuntimeError(
                "Production Scope Integrity syntax failed"
            )
        ...
        if focused.returncode != 0:
            raise RuntimeError(
                "Production Scope Integrity focused tests failed"
            )
        ...
        if compatibility.returncode != 0:
            raise RuntimeError(
                "Production Scope Integrity compatibility tests failed"
            )
        ...
        preflight = run_command(
            ...
            command=[
                ...
                (
                    ...
                    ".read_text(encoding='utf-8'); "
                    "print('pod_cluster_forwarded='+str(p.count('cluster=scope.cluster')>=4)); "
        ...
        if preflight.returncode != 0:
            raise RuntimeError(
                "Production Scope Integrity preflight failed"
            )
        ...
        if authority.returncode != 0:
            raise RuntimeError(
                "Production Scope Integrity authority boundary failed"
            )
        ...
        report.extend(
            [
                "PASSED",
                "",
                "Production Scope Integrity Contract v1 is installed.",
                "",
                "The existing Kubernetes read-only Tool is now cluster-aware:",
                "- matching cluster -> request may proceed",
                "- mismatched configured/requested cluster -> fail closed before HTTP",
                "",
                "Important remaining production work:",
                "- this is not yet multi-cluster routing",
                "- a future Cluster Tool Router must select the correct Kubernetes client from scope.cluster",
                "- Production Shadow should require an explicit cluster identity when multiple clusters are connected",
            ]
        )
        ...
        print("=" * 72)
        print(
            "PRODUCTION SCOPE INTEGRITY CONTRACT V1 PASSED"
        )
        print("=" * 72)
        print("")
        print(
        ...
        write_text(
            ...
            "\n".join(
                [
                    "Production Scope Integrity Contract v1 FAILED",
                    f"GeneratedAt: {datetime.now().astimezone().isoformat()}",
                    "",
