...
VERSION = "production-incident-evidence-recorder-v3"
AFTER_NAME = "production_incident_evidence_recorder_v3_after.txt"
ERROR_NAME = "production_incident_evidence_recorder_v3_error.txt"
...
def discover_compatibility_tests(
    root: Path,
) -> list[str]:
    """
    Discover the compatibility tests that actually exist in this checkout.

    Do not hard-code a guessed Investigation test filename.

    Always require the two canonical replay/dataset suites. Then add up to
    three existing Investigation tests that exercise the read-only Probe or
    production-signal trust boundary.
    """
    ...
    if missing_required:
        raise RuntimeError(
            "Required compatibility test file is missing: "
            + ", ".join(
                str(
                    path.relative_to(
                        root
    ...
    for _negative_score, _name, path in sorted(
        candidates
    )[:3]:
        selected.append(
            path
...
def main() -> int:
    ...
    report = [
        "Production Incident Evidence Recorder v3",
        ...
        "- replay test context provides trace=None",
        "- v3 discovers existing compatibility test files instead of guessing filenames",
    ]
    ...
    try:
        ...
        section(
            report,
            "DISCOVERED COMPATIBILITY TEST FILES",
        )

        for selected_test in compatibility_tests:
            report.append(
                selected_test
            )
        ...
        error_lines = [
            "Production Incident Evidence Recorder v3 FAILED",
            f"GeneratedAt: {datetime.now().astimezone().isoformat()}",
            "",
            "Exception:",
