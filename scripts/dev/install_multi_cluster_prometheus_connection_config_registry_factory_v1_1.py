...
VERSION = "multi-cluster-prometheus-connection-config-registry-factory-v1.1"

AFTER_NAME = (
    "multi_cluster_prometheus_connection_config_registry_factory_v1_1_after.txt"
)

ERROR_NAME = (
    "multi_cluster_prometheus_connection_config_registry_factory_v1_1_error.txt"
)

EXPECTED_HASHES = {'packages/common/src/common/config/settings.py': '74b09338746eaa759f2bfd37e765fbdce7cd10f97475f42923717e5c1dc0626a', 'services/agent_runtime/app/runtime/runtime.py': '124e4860c7002fcdd5e72770d29890e0f2957a8040dbe4a2588bf5422c8ec1a3', 'services/agent_runtime/app/tools/prometheus/tool.py': 'd7ebd8eafbaf1727a34608bb330d6e5633d4c41dbaee2c809b6e3fa9e458da28', 'services/agent_runtime/app/tools/prometheus/router.py': '94a824bd554583481f904672007d3dc33e399d46e02fe3ab7ed2bf7c74b576bf', 'services/agent_runtime/app/tools/factory.py': 'cf0cd9e0871d0b0f5bceb65e58d1be80173cbae001d5d78e1dddc5388819c201'}
...
def verify_hash(
    *,
    ...
    if actual != expected:
        raise RuntimeError(
            (
                f"{relative} changed after the reviewed Prometheus Router/Connection baseline. "
                f"expected_sha256={expected} actual_sha256={actual}. "
                "Refusing stale Prometheus Connection Config installation."
            )
        )
...
def main() -> int:
    ...
    sources = {
        ...
        prometheus_tool_file: PROMETHEUS_TOOL_SOURCE,
        connection_factory_file: (
    ...
    report = [
        "Multi-Cluster Prometheus Connection Config / Registry Factory v1.1",
        f"GeneratedAt: {datetime.now().astimezone().isoformat()}",
        "",
        "Configuration:",
        "- Settings.connections.prometheus_read",
        "- disabled by default",
        "- existing configs/app.yaml remains valid without modification",
        "",
        "Model:",
        "- endpoints[] defines Prometheus/Thanos/Mimir connection identity",
        "- cluster_bindings[] maps Incident cluster -> endpoint",
        "- multiple clusters may explicitly share one endpoint",
        "",
        "Endpoint security:",
        "- clean HTTPS origin only",
        "- authentication is explicit: none OR bearer",
        "- bearer auth requires exactly one env/file credential reference",
        "- raw bearer_token is forbidden in Settings",
        "- optional CA file reference",
        "- TLS verification always enabled for configured multi-cluster endpoints",
        "- mock fallback always disabled",
        "",
        "Ambient-secret safety:",
        "- authentication=none explicitly passes an empty bearer token",
        "- configured no-auth endpoints therefore do not inherit legacy PROMETHEUS_BEARER_TOKEN",
        "",
        "Runtime:",
        "- explicit injected PrometheusClusterRegistry remains authoritative",
        "- otherwise AgentRuntime resolves optional Settings connection registry",
        "- disabled config preserves legacy singleton Prometheus behavior",
        "",
        "CA support:",
        "- PrometheusTool gains optional ca_file without changing existing positional arguments",
        "- live requests use ssl.create_default_context(cafile=...) when configured",
        "",
        "Authority:",
        "- metrics read-plane only",
        "- no Action / Approval / Verification write authority",
    ...
    try:
        ...
        if connection_factory_file.exists():
            raise RuntimeError(
                "Prometheus connection_factory.py already exists; refusing to overwrite an unreviewed factory"
            )

        if test_file.exists():
            raise RuntimeError(
                "Prometheus connection-config test already exists; refusing to overwrite an unreviewed test"
            )
        ...
        if syntax.returncode != 0:
            raise RuntimeError(
                "Prometheus Connection Config syntax failed"
            )

        focused_paths = require_tests(
            root=root,
            label="Prometheus focused",
            relative_paths=[
                (
        ...
        focused = run_command(
            root=root,
            name="Prometheus Connection Config focused suite",
            command=[
                "uv",
                "run",
                "pytest",
        ...
        if focused.returncode != 0:
            raise RuntimeError(
                "Prometheus Connection Config focused tests failed"
            )

        multi_cluster_paths = require_tests(
            root=root,
            label="multi-cluster compatibility",
            relative_paths=[
                (
                    "services/agent_runtime/tests/"
        ...
        if multi_cluster.returncode != 0:
            raise RuntimeError(
                "Prometheus Connection Config multi-cluster compatibility failed"
            )

        settings_paths = require_tests(
            root=root,
            ...
            relative_paths=[
                ...
                (
                    "services/agent_runtime/tests/"
                    "test_kubernetes_preflight_factory_wiring.py"
                ),
                (
                    "services/agent_runtime/tests/"
        ...
        if settings_compat.returncode != 0:
            raise RuntimeError(
                "Prometheus Connection Config settings/security compatibility failed"
            )
        ...
        if runtime_compat.returncode != 0:
            raise RuntimeError(
                "Prometheus Connection Config Runtime compatibility failed"
            )

        preflight = run_command(
            root=root,
            name="Prometheus connection security / disabled-default preflight",
            command=[
                "uv",
                "run",
                "python",
                ...
                (
                    ...
                    "from pathlib import Path; "
                    "s=get_settings(); "
                    "c=s.connections.prometheus_read; "
                    "st=Path(r'packages/common/src/common/config/settings.py').read_text(encoding='utf-8'); "
                    "f=Path(r'services/agent_runtime/app/tools/prometheus/connection_factory.py').read_text(encoding='utf-8'); "
                    "t=Path(r'services/agent_runtime/app/tools/prometheus/tool.py').read_text(encoding='utf-8'); "
                    "rt=Path(r'services/agent_runtime/app/runtime/runtime.py').read_text(encoding='utf-8'); "
                    "print('enabled='+str(c.enabled)); "
                    "print('endpoints='+str(len(c.endpoints))); "
                    "print('bindings='+str(len(c.cluster_bindings))); "
                    "print('raw_secret_field='+str('bearer_token: str' in st)); "
                    "print('ca_support='+str('ssl.create_default_context' in t and 'ca_file: str | None = None' in t)); "
                    "print('runtime_config_factory='+str('create_prometheus_cluster_registry' in rt)); "
                    "assert c.enabled is False; "
                    "assert len(c.endpoints)==0; "
                    "assert len(c.cluster_bindings)==0; "
                    "assert 'class PrometheusReadEndpointConfig' in st; "
                    "assert 'class PrometheusReadMultiClusterConfig' in st; "
                    "assert 'bearer_token: str' not in st; "
                    "assert 'ssl.create_default_context' in t; "
                    "assert 'ca_file: str | None = None' in t; "
                    "assert 'create_prometheus_cluster_registry' in rt; "
                    "assert not any(x in f for x in ['ActionRuntime','ApprovalService','VerificationRuntime','KubernetesProductionExecutor','KubernetesPreflightResolver'])"
                ),
            ],
        )
        ...
        if preflight.returncode != 0:
            raise RuntimeError(
                "Prometheus Connection Config security preflight failed"
            )

        authority = run_command(
            root=root,
            name="Prometheus read-only connection authority boundary",
            command=[
                "uv",
                "run",
                "python",
        ...
        if authority.returncode != 0:
            raise RuntimeError(
                "Prometheus Connection Config authority boundary failed"
            )
        ...
        report.extend(
            [
                "PASSED",
                "",
                "Multi-Cluster Prometheus Connection Config / Registry Factory v1.1 is installed.",
                "",
                "Configured metrics path:",
                "configs/app.yaml -> Settings.connections.prometheus_read -> endpoints + cluster_bindings -> local credential references -> PrometheusTool objects -> PrometheusClusterRegistry -> Multi-Cluster Prometheus Router -> AgentRuntime",
                "",
                "Example future YAML (never place raw token values in app.yaml):",
                "connections:",
                "  kubernetes_read:",
                "    enabled: false",
                "  prometheus_read:",
                ...
                "        base_url: https://metrics.example.internal",
                "        authentication: bearer",
                "        bearer_token_env: PROMETHEUS_READ_TOKEN",
                "        ca_file: /etc/ai-reliability/prometheus/ca.crt",
                "    cluster_bindings:",
                ...
                "- credential rotation without Runtime restart",
                "- endpoint health/readiness startup probes",
                "- write/remediation routing through Prometheus",
                "",
                "Next recommended step:",
                "- Cross-Source Cluster Evidence Consistency Contract v1: prove Kubernetes and Prometheus evidence in one Investigation are from the exact same Incident cluster and reject mismatched tool responses.",
            ]
        )
        ...
        print("=" * 72)
        print(
            "MULTI-CLUSTER PROMETHEUS CONNECTION CONFIG / REGISTRY FACTORY V1.1 PASSED"
        )
        print("=" * 72)
        print()
        print(
        ...
        write_text(
            ...
            "\n".join(
                [
                    "Multi-Cluster Prometheus Connection Config / Registry Factory v1.1 FAILED",
                    (
                        "GeneratedAt: "
