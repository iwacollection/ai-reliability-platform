...
VERSION = "change-investigation-capability-002-v2"

AFTER_NAME = (
    "change_investigation_capability_002_v2_after.txt"
)

ERROR_NAME = (
    "change_investigation_capability_002_v2_error.txt"
)
    ...
    if actual != expected:
        raise RuntimeError(
            (
                ...
                "Refusing stale Change #002 v2 installation."
            )
        )
...
def main() -> int:
    ...
    report = [
        "Change Investigation Capability #002 v2",
        f"GeneratedAt: {datetime.now().astimezone().isoformat()}",
        "",
        "New deterministic evidence:",
        "- Deployment Progressing / Available / ReplicaFailure conditions",
        "- ProgressDeadlineExceeded and ReplicaFailure rollout-failure signals",
        "- generation-observed and rollout-complete state",
        "- bounded incident-window Kubernetes Events for Pod / ReplicaSet / Deployment",
        "- warning count, reasons and bounded event summary",
        "",
        "Event safety/degradation:",
        "- event queries use fixed involvedObject.uid selectors generated by the Tool",
        "- LLM cannot provide the field selector or API path",
        "- event RBAC denial does NOT discard already-trusted revision/image/rollout evidence",
        "- event enrichment becomes unavailable/partial instead",
        "- only GET requests remain permitted",
        "",
        "Causal safety remains unchanged:",
        "- Change evidence alone is still not sufficient causal proof",
        "- existing Epistemic Guard still requires independent trusted support",
        "",
    ...
    try:
        ...
        if syntax.returncode != 0:
            raise RuntimeError(
                "Change #002 v2 syntax verification failed"
            )

        focused = run_command(
            root=root,
            name="Change rollout/events focused suite",
            command=[
                "uv",
                "run",
                "pytest",
        ...
        if focused.returncode != 0:
            raise RuntimeError(
                "Change rollout/events focused tests failed"
            )
        ...
        if compatibility.returncode != 0:
            raise RuntimeError(
                "Change #002 v2 compatibility tests failed"
            )

        preflight = run_command(
            root=root,
            name="Rollout/events semantics preflight",
            command=[
                "uv",
                "run",
                "python",
                ...
                (
                    ...
                    ".read_text(encoding='utf-8'); "
                    "print('rollout_failure_signal='+str('rollout_failure_signal' in t and 'rollout_failure_signal' in p)); "
                    "print('recent_event_summary='+str('recent_event_summary' in t and 'recent_event_summary' in p)); "
                    "print('progress_deadline_prompt='+str('ProgressDeadlineExceeded' in r)); "
                    "assert 'rollout_failure_signal' in t and 'rollout_failure_signal' in p; "
                    "assert 'recent_event_summary' in t and 'recent_event_summary' in p; "
                    "assert 'ProgressDeadlineExceeded' in r"
                ),
            ],
        )
        ...
        if preflight.returncode != 0:
            raise RuntimeError(
                "Change #002 v2 preflight failed"
            )
        ...
        if authority.returncode != 0:
            raise RuntimeError(
                "Change #002 v2 authority boundary failed"
            )
        ...
        report.extend(
            [
                "PASSED",
                "",
                "Change Investigation Capability #002 v2 is installed.",
                "",
                "The existing kubernetes_workload_change probe now includes:",
                "- rollout condition state",
                "- deterministic rollout failure signal",
                "- bounded incident-window Event evidence when RBAC permits",
                "",
                "The capability remains read-only and change evidence alone remains insufficient causal proof.",
                "",
                "Next recommended deterministic step:",
                "add Change benchmark cases for failed rollout events versus unrelated events before spending real model tokens.",
            ]
        )
        ...
        print("=" * 72)
        print(
            "CHANGE INVESTIGATION CAPABILITY #002 V2 PASSED"
        )
        print("=" * 72)
        print("")
        print(
        ...
        write_text(
            ...
            "\n".join(
                [
                    "Change Investigation Capability #002 v2 FAILED",
                    f"GeneratedAt: {datetime.now().astimezone().isoformat()}",
                    "",
