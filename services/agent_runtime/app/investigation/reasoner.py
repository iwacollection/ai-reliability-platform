...
class LLMInvestigationReasoner(
    ...
    def _build_prompt(
        ...
        return (
            ...
            "causation.\n"
            "- kubernetes_config_change: bounded trusted workload-template "
            "configuration-change evidence. It compares current/previous "
            "ConfigMap and Secret references plus config/checksum-like Pod-template "
            "annotations, and may read current ConfigMap metadata such as "
            "resourceVersion/uid. Secret content is never queried and ConfigMap "
            "data values are never exposed to the reasoner. A changed reference "
            "or checksum is temporal/correlation evidence; a current ConfigMap "
            "resourceVersion alone does not prove what historical value changed. "
            "Require independent runtime evidence such as logs, state, or metrics "
            "before accepting a configuration-caused RCA.\n"
