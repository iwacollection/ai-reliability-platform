...
class KubernetesChangeTool(
        ...
        **kwargs: Any,
        ...
        if view not in {
            ...
            raise KubernetesChangeToolError(
                "Kubernetes change view is invalid"
            )
    ...
    async def _collect(
        ...
        *,
        ...
        if view == "config":
            return await self._collect_config_view(
                client=client,
                namespace=namespace,
                deployment=deployment,
                current_replica_set=replica_set,
                previous_replica_set=previous_item,
    ...
    async def _collect_config_view(
        self,
        *,
        client: httpx.AsyncClient,
        namespace: str,
        deployment: Mapping[str, Any],
        current_replica_set: Mapping[str, Any],
        previous_replica_set: Mapping[str, Any] | None,
        revision_before: int | None,
        revision_after: int | None,
    ) -> dict[str, Any]:
        current_refs = self._template_config_references(
            current_replica_set
        )

        previous_refs = (
            self._template_config_references(
                previous_replica_set
            )
            if previous_replica_set is not None
            else {
                "configmaps": [],
                "secrets": [],
                "config_annotation_fingerprint": None,
                "config_annotation_keys": [],
            }
        )

        current_configmaps = current_refs[
            "configmaps"
        ]

        previous_configmaps = previous_refs[
            "configmaps"
        ]

        current_secrets = current_refs[
            "secrets"
        ]

        previous_secrets = previous_refs[
            "secrets"
        ]

        added_configmaps = sorted(
            set(
                current_configmaps
            )
            - set(
        ...
        for name in current_configmaps[
            :16
        ]:
            try:
                payload = await self._get_json(
                    client=client,
                    url=self._config_map_url(
                        namespace=namespace,
                        name=name,
                    ),
                )
            ...
            resource_version_text = (
                self._optional_bounded_text(
                    resource_version,
                    limit=128,
                )
            )

            uid_text = self._optional_bounded_text(
                uid,
                limit=128,
            )
        ...
        if not current_configmaps:
            configmap_metadata_status = "not_applicable"

        elif not metadata_pairs:
            configmap_metadata_status = "unavailable"

        elif metadata_failures:
            configmap_metadata_status = "partial"

        else:
            configmap_metadata_status = "complete"

        metadata_summary = []

        for name, resource_version, uid in metadata_pairs:
            metadata_summary.append(
                (
                    f"{name}:rv={resource_version or '-'}"
                    f":uid={uid or '-'}"
                )
            )

        annotation_before = previous_refs[
            "config_annotation_fingerprint"
        ]

        annotation_after = current_refs[
            "config_annotation_fingerprint"
        ]
        ...
        annotation_changed = (
            ...
            if (
                annotation_before is not None
                or annotation_after is not None
            )
            else None
        )
    ...
    @classmethod
    def _template_config_references(
        cls,
        ...
        template_metadata = template.get(
            "metadata"
        )
        ...
        if isinstance(
            ...
            annotations = template_metadata.get(
                "annotations"
            )
