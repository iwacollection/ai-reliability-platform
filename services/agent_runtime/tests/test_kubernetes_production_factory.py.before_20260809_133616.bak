...
def test_enabled_factory_uses_dedicated_production_identity(
    ...
    budget = budget_service(tmp_path)
    executor = create_kubernetes_production_executor(
        preflight_config(),
        execution_config(),
        environment={
    ...
    assert executor.pilot_budget_service is budget


def test_enabled_factory_requires_ready_pilot_manifest():
    with pytest.raises(
        KubernetesProductionFactoryConfigurationError,
        match="pilot manifest",
    ):
        create_kubernetes_production_executor(
            preflight_config(),
            execution_config(),
...
def test_enabled_factory_requires_durable_pilot_budget():
    with pytest.raises(
        KubernetesProductionFactoryConfigurationError,
        match="budget service",
    ):
        create_kubernetes_production_executor(
            preflight_config(),
            execution_config(),
