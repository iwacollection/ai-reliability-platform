...
class MixedTrustProbeExecutor:
    """
    Return one trusted production signal followed by one explicitly
    untrusted failed signal with deterministic Evidence IDs.
    """
    ...
    async def collect(
        ...
        return EvidenceItem(
            evidence_id="untrusted-evidence-2",
            probe=probe,
            ...
            observed_at=NOW,
            facts={},
            error_code="PrometheusProbeFailed",
        )


@pytest.mark.asyncio
async def test_conclusion_rejects_mixed_trusted_and_untrusted_evidence():
    reasoner = ScriptedReasoner(
        [
            InvestigationDecision(
                hypotheses=[
                ...
                rationale_summary=(
                    "Collect trusted Kubernetes evidence"
                ),
                next_probe=(
                    InvestigationProbe.KUBERNETES_POD_STATE
                ),
            ...
            InvestigationDecision(
                hypotheses=[
                    hypothesis(
                        ...
                        supporting=[
                            "trusted-evidence-1",
                            "untrusted-evidence-2",
                        ],
                        missing=[],
                    )
                ],
                rationale_summary=(
                    "Attempt conclusion using mixed trust evidence"
                ),
                stop=True,
                stop_reason=(
                    InvestigationStopReason.SUFFICIENT_EVIDENCE
    ...
    probes = MixedTrustProbeExecutor()
    ...
    assert (
        result.evidence[0].trusted
        is True
    )

    assert (
        result.evidence[1].trusted
        is False
    )
