...
async def prepared_api(
    ...
    *,
    ...
    runtime.production_pilot_ceremony._clock = clock
    runtime._test_pilot_switch_state = switch_state
...
async def activate_ceremony(
    ...
    response = await client.post(
        ...
        json={
            "executor_operator_id": "test-executor-operator",
        ...
        headers=security.headers(
            OperatorRole.APPROVER,
            include_operator_id=True,
            idempotency_key="ceremony-production-api-0001",
        ),
    )
    assert response.status_code == 200
    runtime._test_pilot_switch_state["value"] = (
        KUBERNETES_PRODUCTION_KILL_SWITCH_DISENGAGED
    )
...
async def test_authenticated_production_resume_replays_once_and_is_auditable(
    ...
    async with api_client(
        ...
        incident_query = await client.get(
            (
    ...
    assert incident_query.status_code == 200
    ...
    ceremony_audit = query[
        "production_pilot_ceremony_audit"
    ]
    assert ceremony_audit[
        "status"
    ] == "activated"
    assert ceremony_audit[
        "execution_id"
...
async def test_ambiguous_production_resume_is_frozen_and_never_verifies(
    ...
    assert "do_not_retry_resume" in ceremony_audit[
        "operator_guidance"
    ]
